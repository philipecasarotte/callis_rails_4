Rails.application.routes.draw do
  ...
  get "login" "login", :controller => "user_sessions", :action => "new"
  get "logout" "logout", :controller => "user_sessions", :action => "destroy"
  resource :user_session
  resources :pages, :as => :paginas
  resources :users, :as => :aniversariantes
  resources :events, :as => :eventos, :collection => { :arquivo => :get } 
  resources :procedures, :as => :procedimentos, :collection => { :search => :get }
